...
function add_task(){
    ...
    if(input_box.value.length != 0 && input_date.value.length != 0){
      ...
      var key = firebase.database().ref().child("To-Do-List").child("B").push().key;
  ...
  function create_unfinished_task(){
    ...
    task_array = [];
    firebase.database().ref("To-Do-List/B/").once('value', function(snapshot) {
  ...
  function finish_edit(task, edit_button){
    ...
    var updates = {};
    updates["/To-Do-List/B/" + key] = task_obj;
    firebase.database().ref().update(updates);
  ...
  function task_delete(task){
    ...
    task_to_remove = firebase.database().ref("/To-Do-List/B/" + key);
